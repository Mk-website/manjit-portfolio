...
export function Select({ label, children, ...props }) {
  return (
    <label className="block">
      {label && <span className="text-sm font-medium text-slate-200">{label}</span>}
      <select {...props} className="mt-1 w-full cursor-pointer appearance-auto rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-slate-100 focus:outline-none focus:ring-2 focus:ring-cyan-500">
        {children}
      </select>
    </label>
  );
